%%
% Runs the 'Exercise' scenario using the 3 methods.
%%
function exercise(duration, variation)

  % Run the 'Exercise' scenario with the estimation method
  heart_rate = estimation_method(duration, variation);

  % Run the 'Exercise' scenario with the Fick's based method
  fick_method(duration, variation);

  % Run the 'Exercise' scenario with the cardiac output principle method
...
function heart_rate = estimation_method(duration, variation)
  ...
  f = figure('name', 'Exercise Scenario - Estimation Method');
...
function fick_method(duration, variation)
  % Exercise parameters
  avg_o2_consumption = 4000;            % Average O2 consumption
  avg_o2_arterial_concentration = 60;   % Average O2 arterial concentration
  avg_o2_venous_concentration = 45;     % Average O2 venous concentration
  ...
  % Plotting
  time = linspace(0, duration, duration);
  f = figure('name', 'Exercise Scenario - Fick Method');
  set(f, 'Position', [0 0 1400 400]);
  grid on;
  subplot(1,3,1);
...
function principle_method(duration, variation, heart_rate)
  % Exercise parameters
  avg_stroke_volume = 136;   % Average stroke volume
  ...
  % Plotting
  time = linspace(0, duration, duration);
  f = figure('name', 'Exercise Scenario - Principle Method');
  set(f, 'Position', [0 0 1400 400]);
  grid on;
  subplot(1,3,1);
  plot(time, heart_rate, 'r');
