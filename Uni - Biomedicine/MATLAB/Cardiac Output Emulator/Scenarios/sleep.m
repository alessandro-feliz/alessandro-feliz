%%
% Runs the 'Sleep' scenario using the 3 methods.
%%
function sleep(duration, variation)

  % Run the 'Sleep' scenario with the estimation method
  heart_rate = estimation_method(duration, variation);

  % Run the 'Sleep' scenario with the Fick's based method
  fick_method(duration, variation);

  % Run the 'Sleep' scenario with the cardiac output principle method
...
function heart_rate = estimation_method(duration, variation)
  % Sleep parameters
  min_rate = 60;       % Minimum heart rate
  max_rate = 70;       % Maximum heart rate
  min_systolic = 85;   % Minimum systolic blood pressure
  max_systolic = 95;  % Maximum systolic blood pressure
  ...
  f = figure('name', 'Sleep Scenario - Estimation Method');
...
function fick_method(duration, variation)
  % Sleep parameters
  avg_o2_consumption = 138;             % Average O2 consumption
  avg_o2_arterial_concentration = 17;   % Average O2 arterial concentration
  avg_o2_venous_concentration = 13;     % Average O2 venous concentration
  ...
  % Plotting
  time = linspace(0, duration, duration);
  f= figure('name', 'Sleep Scenario - Fick Method');
  set(f, 'Position', [0 0 1400 400]);
  grid on;
  subplot(1,3,1);
...
function principle_method(duration, variation, heart_rate)
  % Sleep parameters
  avg_stroke_volume = 65;   % Average stroke volume
  ...
  % Plotting
  time = linspace(0, duration, duration);
  f = figure('name', 'Sleep Scenario - Principle Method');
  set(f, 'Position', [0 0 1400 400]);
  grid on;
  subplot(1,3,1);
  plot(time, heart_rate, 'r');
